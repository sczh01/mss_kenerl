...
int fts_tpd_enable_ps(int enable);
 ...
 int fts_tpd_enable_ps(int enable)
{
	u8 state;
	int ret = -1;
	printk("[proxi_5206]fts_tpd_enable_ps in\n");
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		case SENSOR_DELAY:
			...
			{
				printk("[proxi_5206]Set delay parameter error!\n");
				err = -EINVAL;
			}
			// Do nothing
			break;
        ...
        case SENSOR_ENABLE:
            if((buff_in == NULL) || (size_in < sizeof(int)))
            {
                printk("[proxi_5206]Enable sensor parameter error!\n");
                err = -EINVAL;
            }
            else
            {				
                ...
                if(value)
                {		
                    if((fts_tpd_enable_ps(1) != 0))
                    {
                            printk("[proxi_5206]enable ps fail: %d\n", err); 
                        return -1;
                    }
                }
                else
                {
                    if((fts_tpd_enable_ps(0) != 0))
                    {
                            printk("[proxi_5206]disable ps fail: %d\n", err); 
                        return -1;
                    }
                }
            }
		...
		case SENSOR_GET_DATA:
			...
			{
				    printk("[proxi_5206]get sensor data parameter error!\n");
				err = -EINVAL;
			}
			else
			{
				...
				else
				{
					sensor_data->values[0] = tpd_get_ps_value();
					    printk("[proxi_5206]huang sensor_data->values[0] 1082 = %d\n", sensor_data->values[0]);
			...
			break;
		default:
			    printk("[proxi_5206]proxmy sensor operate function no this parameter %d!\n", command);
			err = -1;
			break;
	}
...
reset_proc:   
 ...
 ///////////////////////////////////////////////
	...
	if (ft_rw_iic_drv_init(i2c_client) < 0)
		printk(TPD_DEVICE, "%s:[FTS] create fts control iic driver failed\n",__func__);
	#endif
...
{
  	printk("FTS I2C Touchscreen Driver (Built %s @ %s)\n", __DATE__, __TIME__);
 ...
 static void tpd_resume( struct early_suspend *h )
 {
  ...
  //char data;
   ...
   // mt_set_gpio_dir(GPIO_CTP_RST_PIN, GPIO_DIR_OUT);
	...
	if((i2c_smbus_read_i2c_block_data(i2c_client, 0x00, 1, &data))< 0)
	{
		printk("resume I2C transfer error, line: %d\n", __LINE__);
	}
	*/
	tpd_up(0,0);
	input_sync(tpd->dev);
