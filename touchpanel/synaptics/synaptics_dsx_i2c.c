...
#define AM(a, args...) printk("amin %s %d " a, __func__, __LINE__, ##args);
...
gprDMABuf_va = (u8 *)dma_alloc_coherent(NULL, 8192, &gprDMABuf_pa, GFP_KERNEL);
        if(!gprDMABuf_va){
...
exit:
 ...
 dma_free_coherent(NULL, 8192, gprDMABuf_va, gprDMABuf_pa);
                gprDMABuf_va = NULL;
                gprDMABuf_pa = NULL;
        }
...
{/*{{{*/
	...
	mutex_lock(&(rmi4_data->rmi4_io_ctrl_mutex));
    AM("rmi4_data: %p\n", rmi4_data);
	gpwDMABuf_va = (u8 *)dma_alloc_coherent(&(rmi4_data->input_dev->dev), 4096, &gpwDMABuf_pa, GFP_KERNEL);
        if(gpwDMABuf_va == NULL){
        	printk("[Error] Allocate DMA I2C Buffer failed!\n");
            retval = -1;
            goto exit;
        }
    ...
    do{
        ...
        AM("data[0]: %d  data[addr]: %p length: %d\n", data[0], data, length);
        memcpy(&buf_va[1], pbuf, length);
        AM("buf_va[1]: 0x%x,  length: %d\n", buf_va[1], length);
        AM("\n");
        for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
...
gpwDMABuf_va = (u8 *)dma_alloc_coherent(NULL, 8192, &gpwDMABuf_pa, GFP_KERNEL);
    if(!gpwDMABuf_va){
    	printk("[Error] Allocate DMA I2C Buffer failed!\n");
		retval = -1;
		goto exit;
...
exit:
	...
	if(gpwDMABuf_va){
        //dma_free_coherent(&(rmi4_data->input_dev->dev), 1024, gpwDMABuf_va, gpwDMABuf_pa);
dma_free_coherent(NULL, 8192, gpwDMABuf_va, gpwDMABuf_pa);
        gpwDMABuf_va = NULL;
        gpwDMABuf_pa = 0;
    }
	mutex_unlock(&(rmi4_data->rmi4_io_ctrl_mutex));
		...
		bool enable)
{
	...
	TPD_DMESG("[maxyu]synaptics_rmi4_sleep_enable 1, rmi4_data->f01_ctrl_base_addr = %d\n", rmi4_data->f01_ctrl_base_addr);
	...
	TPD_DMESG("[maxyu]synaptics_rmi4_sleep_enable 2, rmi4_data->f01_ctrl_base_addr = %d\n", rmi4_data->f01_ctrl_base_addr);
	...
	TPD_DMESG("[maxyu]synaptics_rmi4_sleep_enable 3, device_ctrl = %d\n", device_ctrl);
	...
	///i2c_smbus_write_i2c_block_data(rmi4_data->i2c_client, rmi4_data->f01_ctrl_base_addr, 1, &device_ctrl);  ///added by maxyu 151015 for incell


	TPD_DMESG("[maxyu]synaptics_rmi4_sleep_enable 4,retval = %d\n", retval);
	//retval =1;
...
static ssize_t reset_gpio_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
...
static DEVICE_ATTR(reset_gpio, S_IRUGO | S_IWUGO, reset_gpio_show, reset_gpio_store);
 /**
...
//add start   CUST_EINT_TOUCH_PANEL_SENSITIVE
	mt65xx_eint_set_sens(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_LEVEL_SENSITIVE);
	mt65xx_eint_set_hw_debounce(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_CN);
	//mt65xx_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_POLARITY_LOW, tpd_eint_handler, 0);
	//mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINTF_TRIGGER_FALLING, tpd_eint_handler, 0);
	mt65xx_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN,CUST_EINT_TOUCH_PANEL_SENSITIVE, tpd_eint_handler, 0);
 ...
 */
//extern void lcm_r69006_suspend_reset_low(void);
//extern void lcm_r69006_suspend_power_down(void);
//extern void lcm_r69006_suspend_cmd_setting(void);

void ctp_synaptics_332u_powerdown(void)
{

	mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ZERO);
	hwPowerDown(TPD_POWER_SOURCE, "TP");///added by maxyu  
...
static void synaptics_rmi4_early_suspend(struct early_suspend *h)
{
	...
	TPD_DMESG("[maxyu]synaptics_rmi4_early_suspend fail, rmi4_data->staying_awake 1 = %d\n", rmi4_data->staying_awake);
	...
	TPD_DMESG("[maxyu]synaptics_rmi4_early_suspend fail, rmi4_data->staying_awake 2 = %d\n", rmi4_data->staying_awake);

	//lcm_r69006_suspend_cmd_setting();///added by maxyu 151014 for incell
	msleep(100);
	...
	TPD_DMESG("[maxyu]synaptics_rmi4_early_suspend fail, rmi4_data->staying_awake 3 = %d\n", rmi4_data->staying_awake);
...
exit:
	...
	TPD_DMESG("[maxyu]synaptics_rmi4_early_suspend fail, rmi4_data->staying_awake 4 = %d\n", rmi4_data->staying_awake);
