...
typedef  struct {
	unsigned int hsa;  //FRAME_WIDTH
	unsigned int vsa;  //FRAME_HEIGHT
	unsigned char mode; //CMD_MODE = 0,SYNC_PULSE_VDO_MODE = 1,SYNC_EVENT_VDO_MODE = 2,BURST_VDO_MODE = 3
	unsigned char lane; //LCM_ONE_LANE = 1,LCM_TWO_LANE = 2,LCM_THREE_LANE = 3,LCM_FOUR_LANE = 4,
	unsigned char vsync;//vertical_sync_active
	unsigned char vbp;//vertical_backporch
	unsigned char vfp;//vertical_frontporch
	unsigned char hsync;//horizontal_sync_active
	unsigned char hbp;//horizontal_backporch
	unsigned char hfp;//horizontal_frontporch
	unsigned char div1;//dsi.pll_div1
	unsigned char div2;//dsi.pll_div2
	unsigned char fbk; //dsi.fbk_div
} LCM_setting_timing;
